...
import { Send, MapPin, Users, Clock, Crown, ArrowLeft, MessageSquare, Wifi, WifiOff, RefreshCw } from 'lucide-react';
...
interface ReliableChatRoomProps {
  placeId: string;
  onBack?: () => void;
}
...
const ReliableChatRoom: React.FC<ReliableChatRoomProps> = ({ placeId, onBack }) => {
  ...
  useEffect(() => {
    const fetchPlaceInfo = async () => {
      ...
      try {
        setLoading(true);
        console.log('🔄 Fetching place info for chat:', placeId);
        
        const { data, error } = await supabase
          .from('places')
          .select('id, name, type, merchant_id, latitude, longitude, is_active')
          .eq('id', placeId)
          .eq('is_active', true)
          .single();
        ...
        console.log('✅ Place info loaded for chat:', data);
        setPlaceInfo(data);
      } catch (error) {
  ...
  const handleSendMessage = async () => {
    if (!newMessage.trim() || !isWithinRange || !canSendMessage()) return;
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Button
              ...
              title={`Connection: ${connectionStatus} - Click to reconnect`}
            >
              {getConnectionIcon()}
...
export default ReliableChatRoom;
